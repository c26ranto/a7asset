...
class AuthRepository {
  ...
  Future<void> signIn({required String username, required String password, required String database}) async {
    try {
      return await authService.signIn(username: username, password: password, database: database);
      ...
      final data = (decodeData as Map).containsKey("Message") ? decodeData["Message"] : decodeData;
      throw CustomError(errorCode: e.errorCode, errorMessage: data);
    } catch (e) {
      throw CustomError(errorMessage: AppErrorMessage.internalServerError, errorCode: AppErrorCode.internalServerError);
    }
  }
